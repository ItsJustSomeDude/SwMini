...
//		LOGD("Trying to find resource in dir '%s' (%li)", base_path, base_len);
	...
	if (asset != NULL) {
		...
		LOGD("Found resource '%s' in APK", input);
		AAsset_close(asset);
		return ASSET;
	}
...
/** For MiniPaths with the "resource" type, this will change it to "Asset", "Resource, or "Error". */
void locate_resource(MiniPath *path) {
	if (path->type != RESOURCE)
		return; /* Type is already found. */

	char tmp[sizeof(path->path)];
	MiniPathType type = find_resource(tmp, sizeof(tmp), path->path);
	if (type == ERROR) {
	...
	strncpy(path->path, tmp, sizeof(path->path));
	path->type = type;
	path->path_length = strlen(tmp);
}
